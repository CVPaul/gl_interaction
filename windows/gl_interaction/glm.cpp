...
GLMmodel*
glmCopyOBJ(const GLMmodel* model); is added on 2014-12 by myself
*/
...
/*
@ brief: copy a GLMmodel(OBJ) with memory allocate (a new instance)
@ added by xql on 2015-02-06
@ param[in] model the source model
@ return the new instance model
*/
GLMmodel*
glmCopyOBJ(const GLMmodel* model)
{
	if (!model) return NULL;
	// some common operation
	GLMmodel* copymodel = (GLMmodel*)malloc(sizeof(GLMmodel));
	copymodel->numvertices = model->numvertices;
	copymodel->numnormals = model->numnormals;
	copymodel->numtexcoords = model->numtexcoords;
	copymodel->numtriangles = model->numtriangles;
	copymodel->numfacetnorms = model->numfacetnorms;
	copymodel->nummaterials = model->nummaterials;
	// init adress with NULL:
	copymodel->facetnorms = NULL;
	copymodel->groups = NULL;
	copymodel->mtllibname = NULL;
	copymodel->materials = NULL;
	copymodel->normals = NULL;
	copymodel->pathname = NULL;
	copymodel->texcoords = NULL;
	copymodel->triangles = NULL;
	copymodel->vertices = NULL;
	...
	if (model->pathname)
	{
		int size = strlen(model->pathname) + 1;
		if (size <= 1)
		{
			copymodel->pathname = NULL;
		}
		...
		{
			copymodel->pathname = (char*)malloc(size*sizeof(char));
			if (!copymodel->pathname)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-pathname)\n");
				glmDelete(copymodel); //clear the allocated memory
				return NULL;
			}
			memcpy(copymodel->pathname, model->pathname, size);
		}
	}
	if (model->mtllibname)
	{
		int size = strlen(model->mtllibname) + 1;
		if (size <= 1)
		{
			copymodel->mtllibname = NULL;
		}
		...
		{
			copymodel->mtllibname = (char*)malloc(size*sizeof(char));
			if (!copymodel->mtllibname)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-mtlibname)\n");
				glmDelete(copymodel); //clear the allocated memory
				return NULL;
			}
			memcpy(copymodel->mtllibname, model->mtllibname, size);
		}
	}
	if (model->vertices)
	{
		int size = copymodel->numvertices + 1;
		if (size <= 1)
		{
			copymodel->vertices = NULL;
		}
		...
		{
			copymodel->vertices = (float*)malloc(3 * size*sizeof(float));
			if (!copymodel->vertices)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-vertices)\n");
				glmDelete(copymodel); //clear the allocated memory
				return NULL;
			}
			memcpy(copymodel->vertices, model->vertices, 3 * size*sizeof(float));
		}
	}
	if (model->normals)
	{
		int size = copymodel->numnormals + 1;
		if (size <= 1)
		{
			copymodel->normals = NULL;
		}
		...
		{
			copymodel->normals = (float*)malloc(3 * size*sizeof(float));
			if (!copymodel->normals)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-normals)\n");
				glmDelete(copymodel); //clear the allocated memory
				return NULL;
			}
			memcpy(copymodel->normals, model->normals, 3 * size*sizeof(float));
		}
	}
	if (model->texcoords)
	{
		int size = copymodel->numtexcoords + 1;
		if (size <= 1)
		{
			copymodel->texcoords = NULL;
		}
		...
		{
			copymodel->texcoords = (float*)malloc(2 * size*sizeof(float));
			if (!copymodel->texcoords)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-texcoords)\n");
				glmDelete(copymodel); //clear the allocated memory
				return NULL;
			}
			memcpy(copymodel->texcoords, model->texcoords, 2 * size*sizeof(float));
		}
	}
	if (model->facetnorms)
	{
		int size = copymodel->numfacetnorms + 1;
		if (size <= 1)
		{
			copymodel->facetnorms = NULL;
		}
		...
		{
			copymodel->facetnorms = (float*)malloc(3 * size*sizeof(float));
			if (!copymodel->facetnorms)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-normals)\n");
				glmDelete(copymodel); //clear the allocated memory
				return NULL;
			}
			memcpy(copymodel->facetnorms, model->facetnorms, 3 * size*sizeof(float));
		}
	}
	if (model->triangles)
	{
		int size = copymodel->numtriangles;
		if (size <= 0)
		{
			copymodel->triangles = NULL;
		}
		...
		{
			copymodel->triangles = (GLMtriangle*)malloc(size*sizeof(GLMtriangle));
			if (!copymodel->triangles)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-triangles)\n");
				glmDelete(copymodel); //clear the allocated memory
				return NULL;
			}
			memcpy(copymodel->triangles, model->triangles, size*sizeof(GLMtriangle));
		}
	}
	if (model->materials)
	{
		copymodel->materials = (GLMmaterial*)malloc(copymodel->nummaterials*sizeof(GLMmaterial));
		int csz = 0;
		for (i = 0; i < copymodel->nummaterials; i++)
		{
			csz = strlen(model->materials[i].name) + 1;
			if (csz <= 1)
			{
				copymodel->materials[i].name = NULL;
			}
			else
			{
				copymodel->materials[i].name = (char*)malloc(csz*sizeof(char));
				memcpy(copymodel->materials[i].name, model->materials[i].name, csz*sizeof(char));

				memcpy(copymodel->materials[i].ambient, model->materials[i].ambient, 4 * sizeof(GLfloat));
				memcpy(copymodel->materials[i].ambient, model->materials[i].ambient, 4 * sizeof(GLfloat));
				memcpy(copymodel->materials[i].ambient, model->materials[i].ambient, 4 * sizeof(GLfloat));
				memcpy(copymodel->materials[i].ambient, model->materials[i].ambient, 4 * sizeof(GLfloat));
			}
	...
	int gsz = 0;
	group = model->groups;
	GLMgroup** cpgroup = &(copymodel->groups);
	while (group)
	{
		*cpgroup = (GLMgroup*)malloc(sizeof(GLMgroup));
		(*cpgroup)->next = NULL; // be careful !!!!
		(*cpgroup)->material = NULL;
		(*cpgroup)->name = NULL;
		(*cpgroup)->triangles = NULL;
		if (!(*cpgroup))
		{
			printf("Error! not enough memory avilable!(in copyOBJ()-groups)\n");
			glmDelete(copymodel); //clear the allocated memory
			return NULL;
		}
		if (group->name)
		{
			(*cpgroup)->name = (char*)malloc(gsz*sizeof(char));
			if (!(*cpgroup)->name)
			{
				printf("Error! not enough memory avilable!(in copyOBJ()-groups)\n");
				glmDelete(copymodel); //clear the allocated memory
			...
			memcpy((*cpgroup)->name, group->name, gsz);
			(*cpgroup)->material = group->material;
			(*cpgroup)->numtriangles = group->numtriangles;
			if ((*cpgroup)->numtriangles > 0)
			{
				(*cpgroup)->triangles = (GLuint*)malloc(sizeof(GLuint)*(*cpgroup)->numtriangles);
				if (!(*cpgroup)->triangles)
				{
					printf("Error! not enough memory avilable!(in copyOBJ()-groups-triangles)\n");
					glmDelete(copymodel); //clear the allocated memory
					return NULL;
				}
				memcpy((*cpgroup)->triangles, group->triangles, sizeof(GLuint)*(*cpgroup)->numtriangles);
			}
		}
		group = group->next;
		cpgroup = &((*cpgroup)->next);
	}
	return copymodel;
}
...
/*
@ glmdrawGroup(): add by xql 2015-01-09(small modifies)
retrival the group and draw!
@ param[in]: groupname the name of the group to draw;
@ param[in]: the same as glmdraw()
@ param[in]: mode the same as glmdraw()
*/
GLvoid glmdrawGroup(char* groupname, GLMmodel* model, GLuint mode)
{
	static GLuint i;
	static GLMgroup* group;
	static GLMtriangle* triangle;
	...
	group = model->groups;
	while (group&&strcmp(group->name, groupname) != 0)group = group->next;
	if (!group)
	{
		printf("can not find the group %s,so retrun( from glmdrawGroups())!\n", group);
		return;
	}
	if (mode & GLM_MATERIAL) {
		material = &model->materials[group->material];
